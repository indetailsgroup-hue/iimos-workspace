...
| Wave 2 | Committed as `ae14fb66` | `coverage.py` +88/−35, `test_release.py` +294/−0. The reachability table became a derivation. **No committed test mutated that derivation.** An earlier edition of this closeout called it "a mutation-tested guard"; that overstated it. The mutation testing was performed by a reviewer in a scratchpad that does not survive, the same limitation this ledger already records for Task 7's figure-derivation guard. `git grep` for a mutation test over the derivation returns nothing at `5e17afba`. **Task 9 closed the gap:** `b50b0c96` added two guard tests that drive the derivation and fail when a reason is added without a demonstration, verified independently by mutating `coverage.EVIDENCE_GATE_REASONS` before the test module imports it — clean 5/5 OK, added reason 2 failures, reason dropped from discovery 2 failures. Both were written after the implementation and pass by construction, which is disclosed rather than implied. So the phrase is true from `b50b0c96` onward and was not true before it; stating only the retraction would have created the opposite inaccuracy. |
...
- **The `51c6428b` commit message contains a claim this closeout falsifies, and it is recorded here because a commit message cannot be corrected in place.** That message says the tracked files outside the registry root *"are documents and workflows where the conversion is harmless."* Re-measured at accepted HEAD by simulating a fresh checkout with `git checkout-index`: of 196 tracked files, 113 differ from the worktree on checkout, and **9 of those are `.json`/`.jsonl` data files**, six of them under `data/component-master/`. More consequentially, `docs/reports/2026-07-26-global-connector-registry-baseline-adoption-manifest.json` publishes 77 path-and-SHA-256 pairs, and **none of the 77 reproduce on a fresh checkout**; 74 of the 77 still match the worktree, and the other 3 have legitimately moved on under later commits. The owner ruled the *scope* of `51c6428b` to be the registry root only and **that ruling stands unchanged**; what is falsified is the word "harmless", not the scope. No release digest is affected, and the reason is narrower than "the release reads only that directory": the pinning is per-glob, so `.gitattributes` and `.gitignore` **inside** `data/component-master/registry/v1` still convert on checkout, at 1,388 → 1,413 and 16 → 17 bytes. What holds is that the release reads only `*.json` and `*.jsonl` under that root and exactly those two globs are pinned — demonstrated the strong way by building a release from the simulated fresh checkout and getting the same 4,428-byte payload and the same digest. Whether the Task 1 baseline manifest gets its own wave is an open owner decision.
- **The `51c6428b` figure of "76 tracked files" is not derivable from the repository.** It counted files that already carried CR in *this particular working tree*, which is a property of how that tree was created, not of any commit. Zero blobs carry CR at any revision in the range, so no checkout of any commit reproduces the count; the same worktree count at accepted HEAD is 81. In fairness to that message, it did say "in working trees" rather than presenting the count as a repository fact. The defect is narrower than a first edition of this closeout stated: the figure is unreproducible by a reader, not mislabelled by its author.
- **A plan-versus-implementation conflict blocks Task 9 as the plan is currently written, and it has been reproduced end to end with the plan's own command.** The plan's own wording is prose, not a schema — *"Each row records publisher, official URL, edition when printed, region, language, access date, rights state and one of `DISCOVERED`, `SOURCE_BLOCKED`, `DORMANT_OR_DEFUNCT` or `REVIEWED`."* It names seven concepts and four states and no `sha256`. All three collide with this module, and the collision does not depend on what the seven concepts are eventually named: a declared denominator row holds **exactly** `blocked_reason`, `sha256`, `source_id` and `state`, so any seven additional fields under any spelling are refused; **all four** of the plan's states are outside the two-value vocabulary `("BLOCKED", "REGISTERED")` — `SOURCE_BLOCKED` is not `BLOCKED`, and the check is literal membership; and `SourceDenominatorEntry` requires 64 lowercase hex where the plan supplies nothing. A row written to the plan's description is refused, and the plan's own Step 4 invocation `check_coverage.py --root data/component-master/registry/v1 --fail-on-unclassified` exits `2` where the plan states *"Expected: exit 0."* **A first edition of this closeout said "three of the four states" and rendered the seven concepts as if they were field names quoted from the plan. Both were wrong, the first understated the conflict, and they are corrected here.** This is not a defect in `26d344e3` — the reader refuses loudly and names what to change — but Task 9's brief must reconcile the row schema, the state vocabulary and the `sha256` requirement **before** any Task 9 work starts.
...
- **The same absence reaches the Origin column, and that is the more serious half of it.** The two waves labelled OWNER-RULED, the review verdicts in the chronology table, and the statement that `.gitattributes` was authored by the orchestrator rather than an implementer all rest on the session record alone. `grep` over the whole parent tree finds no Task 8 owner-ruling artifact. `51c6428b`'s commit message does contain the contemporaneous line *"Scope is this directory only, by owner ruling"*; **`26d344e3`'s message carries no owner attribution at all** and reads as an engineering decision. Git author metadata cannot corroborate the orchestrator/implementer distinction either, because every commit in the range carries one identity as both author and committer. The finding *counts* in the chronology are corroborated by the commit bodies — `af351f06` says "Close five review findings" and `ae14fb66` says "Close three narrow findings from the second review wave" — but deciding authority is exactly the axis Task 7's ledger says must not blur, and here it is asserted in bold with no artifact behind it. A reader who wants to check who decided wave B cannot.
- **A file at the registry root that is not `*.jsonl` is skipped silently, and no message says so.** `discover_registry_root` iterates `rglob("*.jsonl")`, so a well-formed item object written as `items.json`, or a stray `notes.txt`, leaves `check_coverage --fail-on-unclassified` at exit `0` with `discovered_item_count: 0`, while the same content in `foo.jsonl` exits `2` and names the file. The loud-refusal guarantee recorded above is therefore scoped to the `.jsonl` extension, not to the directory. This is harmless while the root is empty, but Tasks 9–12 write into that root. On a case-sensitive filesystem `ITEMS.JSONL` would join the silent column; on this host the glob is case-insensitive and it is caught.
