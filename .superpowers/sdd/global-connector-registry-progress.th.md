...
| Wave 2 | commit เป็น `ae14fb66` | `coverage.py` +88/−35, `test_release.py` +294/−0 ตารางความเข้าถึงได้กลายเป็นการ derive **ไม่มีเทสต์ที่ commit ไว้ตัวใดทำ mutation กับการ derive นั้น** ฉบับก่อนหน้าของการปิดงานนี้เรียกมันว่า "guard ที่ผ่าน mutation testing" ซึ่งเกินจริง การทำ mutation ทำโดย reviewer ใน scratchpad ที่ไม่หลงเหลือ เป็นข้อจำกัดแบบเดียวกับที่ ledger นี้บันทึกไว้แล้วสำหรับ figure-derivation guard ของงานที่ 7 `git grep` หาเทสต์ mutation ของการ derive นี้ที่ `5e17afba` ไม่พบอะไรเลย **งานที่ 9 ปิดช่องว่างนี้แล้ว:** `b50b0c96` เพิ่ม guard test สองตัวที่ขับการ derive และล้มเมื่อมีการเพิ่มเหตุผลโดยไม่มี demonstration ตรวจสอบอิสระด้วยการ mutate `coverage.EVIDENCE_GATE_REASONS` ก่อนที่โมดูลเทสต์จะ import — สะอาด 5/5 OK, เพิ่มเหตุผล 2 failures, ตัดเหตุผลออกจาก discovery 2 failures ทั้งสองตัวเขียนขึ้นหลัง implementation และ pass by construction ซึ่งบอกไว้ตรง ๆ ไม่ใช่ปล่อยให้เดา ดังนั้นประโยคนั้นเป็นจริงตั้งแต่ `b50b0c96` เป็นต้นไป และไม่จริงก่อนหน้านั้น การเขียนเฉพาะการถอนคำจะสร้างความไม่ถูกต้องในทางกลับกันแทน |
