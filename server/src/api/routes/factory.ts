...
export function factoryRouter(deps: FactoryRouterDeps): Router {
   ...
   * GET /factory/jobs/:jobId/export/:exportId/download - Download export file
   *
   * S18: legacy endpoint เคยแต่ง G-code ปลอมพร้อม SHA จริง — ตอบ 501 ตรง ๆ
   * แทนการหลอกเครื่องจักร เส้นทางจริงคือ factory-api (Supabase edge):
   * GET /factory/jobs/:jobId/export → signed URL ของ packet ที่ RELEASED แล้ว
   */
  router.get('/jobs/:jobId/export/:exportId/download', async (_req: Request, res: Response) => {
    res.status(501).json({
      ok: false,
      error: 'E_NOT_IMPLEMENTED',
      message:
        'เส้นทางนี้ยังไม่สร้างไฟล์จริง — ดาวน์โหลด packet ผ่าน factory-api: ' +
        'GET /factory/jobs/:jobId/export (ต้องเป็นงานสถานะ RELEASED จะได้ signed URL) ' +
        'แล้วตรวจไฟล์ด้วย POST /factory/jobs/:jobId/verify',
    });
  });
