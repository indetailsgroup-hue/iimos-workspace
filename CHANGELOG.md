...
### Added
- **Migration 0186 — e-Tax Compliance Dashboard View** (`v_etax_compliance_dashboard`): Read-only VIEW joining `etax_submissions`, `etax_submission_audit_log`, and `invoice_notifications` to surface per-org e-Tax compliance metrics. Key columns: `total_submissions`, `submitted_count`, `failed_count`, `cancelled_count`, `queued_count`, `submitting_count`, `success_rate` (%), `avg_attempt_count`, `max_attempt_count`, `submissions_with_pdf_downloaded`, `pdf_success_rate` (%), `overdue_invoice_count`, `overdue_with_pending_etax`, `failed_last_24h`, `last_submission_at`, `last_audit_event_at`, `last_failed_at`, `oldest_unresolved_failed_at`. **Success rate** = submitted ÷ (submitted + failed + cancelled) × 100 — in-flight rows excluded from denominator. **overdue_with_pending_etax** counts overdue invoices (active, non-dismissed, non-snoozed notifications) that also have a non-submitted/non-cancelled etax record. RPCs: `rpc_etax_compliance_dashboard()` (authenticated, org-scoped via `get_user_org_id()`); `rpc_etax_compliance_all_orgs(p_min_failed_last_24h)` (service-role only, sorted by failing-first). Supporting indexes: `idx_etaxsub_org_status_time`, `idx_etaxsub_org_pdf_status`, `idx_notif_org_type_status`. View is NOT exposed directly to authenticated — all access mediated by SECURITY DEFINER RPCs.
- **Migration 0187 — Materialized View + pg_cron Refresh** (`mv_etax_compliance_dashboard`): Materialized copy of `v_etax_compliance_dashboard` for high-traffic dashboards. Unique index `uq_mv_etax_compliance_org` on `org_id` enables `REFRESH MATERIALIZED VIEW CONCURRENTLY` (no exclusive table lock). `fn_refresh_etax_compliance_mv(triggered_by)` SECURITY DEFINER function executes CONCURRENT refresh, logs `duration_ms` + `row_count` to `etax_compliance_mv_refresh_log`. pg_cron job `refresh-etax-compliance-mv` runs `*/15 * * * *` (every 15 min). Staleness: up to 15 min — use live view RPCs from 0186 when real-time data is required. RPCs: `rpc_etax_compliance_dashboard_cached()` (authenticated, org-scoped, includes `mv_last_refreshed_at` + `mv_age_seconds`); `rpc_etax_compliance_all_orgs_cached(p_min_failed_last_24h)` (service-role); `rpc_refresh_etax_compliance_mv()` (service-role, manual warm-up). Monitoring view `v_mv_refresh_lag` surfaces `lag_seconds` and `freshness_status` (fresh/stale/critical). Refresh tracking table `etax_compliance_mv_refresh_log` (pruned to last 1000 rows per cycle).
- **Test suite `0186_compliance_dashboard.test.ts`** (937 lines, Groups A–F, 37 tests): org isolation (6 tests — cross-org blocked, empty org, unauthenticated); success_rate accuracy (6 tests — 100%, 60%, NULL for in-flight, queued excluded from denominator); overdue_with_pending_etax logic (11 tests — queued/failed/submitting counted, submitted/cancelled/no-etax not counted, dismissed/snoozed/expired-snooze/due_soon edge cases); rpc_etax_compliance_all_orgs ordering + filter (7 tests — DESC sort, min_failed_last_24h filter, 24h window, authenticated blocked); metric accuracy (7 tests — avg/max attempt, pdf_rate, NULL pdf_rate, failed_last_24h window, last_audit_event_at, oldest_unresolved); column completeness (7 tests — all columns present, numeric types, view blocked from authenticated, last_submission_at correctness).
...
### Added
- **Stage 130** (`curvedPanelDxfPipeline.smoke.test.ts`): asserts that `qty=715` curved
  panels (715 = 13 × 55) are packed into exactly 13 full sheets with no remainder;
  every sheet holds exactly 55 placements with `isCurved=true`; a DXF loop across all
  13 sheets asserts `PARTS_CURVED=220` (55 × 4) on each — continues the full-sheet
  sequence …→ 124 → 126 → 128 → **130**.
- **Stage 131** (`curvedPanelDxfPipeline.smoke.test.ts`): determinism guard for the
  Stage 128 fixture (`qty=660`); two consecutive `runNesting` calls produce identical
  `sheets[11].placements[0]` `partId`, `x`, `y`, and `rotation` — extends the
  determinism series (85, 95, 99, 103, 107, 111, 114, 117, 119, 121, 123, 125, 127,
  129, **131**).
- Updated smoke-test JSDoc: range extended to `Stages 22 – 131`; table rows 130–131
  added before the FFDH Constants separator.
- Updated `buildDxfSheets.ts` cross-reference comment: `(Stages 7 – 131)`.

### Tests
- Smoke suite: **384 tests**, all passing.


## [12.4.0] - 2026-08-27

### Added
- **Stage 128** — Twelve full sheets, no remainder (`qty=660 = 12×55`): asserts `sheets.length===12`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all twelve sheets; continues the full-sheet sequence
  … → 124 (10) → 126 (11) → 128 (12).
- **Stage 129** — Eleven-full-sheet determinism guard (`qty=605`): runs `runNesting` twice with
  the Stage 126 fixture and asserts `sheets[10].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series
  (Stages 85, 95, 99, 103, 107, 111, 114, 117, 119, 121, 123, 125, 127, **129**).
- Updated smoke-test JSDoc: stage-table rows 128–129; range line updated to `Stages 22–129`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–129)`.

### Tests
- **382 tests passing** (was 380 after v12.3.0).
...
### Added
- **Stage 126** — Eleven full sheets, no remainder (`qty=605 = 11×55`): asserts `sheets.length===11`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all eleven sheets; continues the full-sheet sequence
  … → 122 (9) → 124 (10) → 126 (11).
- **Stage 127** — Ten-full-sheet determinism guard (`qty=550`): runs `runNesting` twice with
  the Stage 124 fixture and asserts `sheets[9].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series
  (Stages 85, 95, 99, 103, 107, 111, 114, 117, 119, 121, 123, 125, **127**).
- Updated smoke-test JSDoc: stage-table rows 126–127; range line updated to `Stages 22–127`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–127)`.

### Tests
- **380 tests passing** (was 378 after v12.2.0).
...
### Added
- **Stage 124** — Ten full sheets, no remainder (`qty=550 = 10×55`): asserts `sheets.length===10`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all ten sheets; continues the full-sheet sequence
  … → 120 (8) → 122 (9) → 124 (10).
- **Stage 125** — Nine-full-sheet determinism guard (`qty=495`): runs `runNesting` twice with
  the Stage 122 fixture and asserts `sheets[8].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series
  (Stages 85, 95, 99, 103, 107, 111, 114, 117, 119, 121, 123, **125**).
- Updated smoke-test JSDoc: stage-table rows 124–125; range line updated to `Stages 22–125`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–125)`.

### Tests
- **378 tests passing** (was 376 after v12.1.0).
...
### Added
- **Stage 122** — Nine full sheets, no remainder (`qty=495 = 9×55`): asserts `sheets.length===9`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all nine sheets; continues the full-sheet sequence
  …→ 118 (7) → 120 (8) → 122 (9).
- **Stage 123** — Eight-full-sheet determinism guard (`qty=440`): runs `runNesting` twice with
  the Stage 120 fixture and asserts `sheets[7].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series
  (Stages 85, 95, 99, 103, 107, 111, 114, 117, 119, 121, **123**).
- Updated smoke-test JSDoc: stage-table rows 122–123; range line updated to `Stages 22–123`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–123)`.

### Tests
- **376 tests passing** (was 374 after v12.0.0).
...
### Added
- **Stage 118** — Seven full sheets, no remainder (`qty=385 = 7×55`): asserts `sheets.length===7`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all seven sheets; continues the full-sheet sequence
  108 (2) → 110 (3) → 112 (4) → 115 (5) → 116 (6) → 118 (7).
- **Stage 119** — Six-full-sheet determinism guard (`qty=330`): runs `runNesting` twice with
  the Stage 116 fixture and asserts `sheets[5].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series
  (Stages 85, 95, 99, 103, 107, 111, 114, 117, **119**).
- Updated smoke-test JSDoc: stage-table rows 118–119; range line updated to `Stages 22–119`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–119)`.

### Tests
- **372 tests passing** (was 370 after v11.8.0).
...
### Added
- **Stage 114** — Four-full-sheet determinism guard (`qty=220`): runs `runNesting` twice with
  the Stage 112 fixture and asserts `sheets[3].placements[0]` partId, x, y, rotation are
  identical across both runs; extends the determinism series (Stages 85, 95, 99, 103, 107, 111).
- **Stage 115** — Five full sheets, no remainder (`qty=275 = 5×55`): asserts `sheets.length===5`,
  every sheet holds exactly 55 placements (all `isCurved=true`), and a DXF loop verifies
  `PARTS_CURVED=220` on all five sheets; continues the full-sheet sequence
  108 (2) → 110 (3) → 112 (4) → 115 (5).
- Updated smoke-test JSDoc: stage-table rows 114–115; range line updated to `Stages 22–115`;
  `buildDxfSheets.ts` cross-reference updated to `(Stages 7–115)`.

### Tests
- **368 tests passing** (was 366 after v11.6.0).
...
#### Stage 110 — Three full sheets (qty=165, no remainder)
- Smoke test asserting that `qty=165` curved panels produce exactly **3 sheets** with no remainder
  (165 = 3 × 55).
- All three sheets (`sheets[0]`, `sheets[1]`, `sheets[2]`) hold exactly **55 placements**,
  all `isCurved=true`.
- DXF `PARTS_CURVED = 220` (55 × 4) asserted independently on **all three** sheets via a loop.
- Continues the full-sheet boundary sequence: Stage 108 (n=2) → Stage 110 (n=3).

#### Stage 111 — Two-full-sheet determinism guard
- Regression guard: `runNesting` called twice with the Stage 108 fixture (`qty=110`) produces
  **identical** `sheets[1].placements[0]` `partId`, `x`, `y`, and `rotation`.
- Validates determinism at the 2-full-sheet boundary (110 = 2 × 55).
- Extends the determinism series: Stages 85, 95, 99, 103, 107, **111**.

#### JSDoc updates
- Stage-table rows 110–111 added to smoke test file JSDoc.
- JSDoc range line updated: `Stages 22–109` → `Stages 22–111`.
- `buildDxfSheets.ts` cross-reference updated: `(Stages 7–109)` → `(Stages 7–111)`.

### Changed
- Smoke test suite now contains **364 tests** (Stages 1–111 + supporting helpers).
...
#### Stage 108 — Two full sheets (qty=110, no remainder)
- Smoke test asserting that `qty=110` curved panels produce exactly **2 sheets** with no remainder
  (110 = 2 × 55).
- Both `sheets[0]` and `sheets[1]` are asserted to hold exactly **55 placements**,
  all `isCurved=true`.
- DXF `PARTS_CURVED = 220` (55 × 4) asserted independently on **both** sheets.
- 2-sheet analogue of Stage 100 (6 full sheets) — validates the full-sheet boundary at n=2.

#### Stage 109 — Two full sheets + 1 overflow (qty=111)
- Smoke test asserting that `qty=111` curved panels produce exactly **3 sheets**
  (111 = 55 + 55 + 1).
- `sheets[0]` and `sheets[1]` each hold 55 placements; `sheets[2]` holds exactly **1 placement**.
- Overflow panel on sheet-3 asserted at `x=10`, `y=10`, `rotation=90`.
- All placements across all three sheets are `isCurved=true`.
- Mirrors Stage 106 (1-over at the 1-sheet boundary) at the 2-sheet boundary.

#### JSDoc updates
- Stage-table rows 108–109 added to smoke test file JSDoc.
- JSDoc range line updated: `Stages 22–107` → `Stages 22–109`.
- `buildDxfSheets.ts` cross-reference updated: `(Stages 7–107)` → `(Stages 7–109)`.

### Changed
- Smoke test suite now contains **362 tests** (Stages 1–109 + supporting helpers).
