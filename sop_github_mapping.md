...
| n:157-mod | thai_mcp_layer_implementation.html | HTML (modified) | Agent Orchestration Flow diagram added: 5-agent chain Sale→Designer→PM→Procurement→Factory, MCP tool calls, approval gates, back-channels, tool summary table | monolith-mcp-layer_tasks.md | 2026-09-04 |
| n:159 | thai_knowledge_base_approval_workflow.html | HTML | Task 1.4 Gate-1 SME workflow; 8-step approval process, section review cards, SME assignment table, decision flow, version control, re-embed trigger | _knowledge-export.json, monolith-mcp-layer_tasks.md | 2026-09-04 |
| n:160 | thai_ai_agent_evaluation_metrics.html | HTML | UAT Phase E evaluation; 5 agents, KPI benchmarks, PFMEA test cases (Sev≥7), 4-level rubric, Gate-1 scorecard | _knowledge-export.json (pfmeaRiskRows 122 rows), raciMap | 2026-09-04 |
| n:161 | thai_automation_layer_spec.html | thai_automation_layer_spec.html | NEW | Automation Layer Spec — LINE Messaging API + MCP Integration (5 tabs, 7 Flex templates, Python scaffold) |
| n:162 | thai_installation_agent_spec.html | HTML | Installation Agent Spec — Agent #6; 6 tabs; system prompt, KPI benchmarks (6 KPIs), 12-item site checklist, defect capture flow (6 steps), PFMEA test cases TC-INSTALL-01 to 05 (TC-02 + TC-05 Critical score=4); accent purple #5b4fcf | _knowledge-export.json (pfmeaRiskRows), thai_ai_agent_evaluation_metrics.html | 2026-09-04 |
| n:163 | thai_line_oa_customer_journey.html | HTML | LINE OA Customer Journey — 10 stages (Add LINE OA→Complete); 6 tabs; Flex Message mockups (5 stages), Rich Menu 3×2 grid + 5 variants, LIFF 5 apps, auto-reply rules, notify_customer() code, go-live checklist; LINE Notify DEPRECATED March 2025 | thai_automation_layer_spec.html (LINE Messaging API), flex_builder.py | 2026-09-04 |
| n:164 | thai_installation_agent_fieldapp.html | HTML | PWA Field App — 5 screens (GPS Check-In, Defect Capture, Site Checklist, Customer Sign-off, Job Summary); canvas signature pad; photo upload ×3; 12-item checklist; PFMEA Sev selector; MCP job_scheduled/job_complete events; Service Worker via Blob URL; accent purple #5b4fcf | thai_installation_agent_spec.html (KPI/checklist), _knowledge-export.json (pfmeaRiskRows) | 2026-09-04 |
| n:163-mod | thai_line_oa_customer_journey.html | HTML | UPDATED — LIFF TypeScript Implementation card added in Tab 06: liff-status.ts (LIFF_ID_STATUS + Supabase real-time projects table, RLS, progress bar) + liff-approval.ts (LIFF_ID_APPROVAL + design_approvals table, submitApproval(), subscribeApproval(), liff.closeWindow()) | _knowledge-export.json, thai_automation_layer_spec.html | 2026-09-04 |
| n:165 | thai_pfmea_live_dashboard.html | HTML | PFMEA Live Dashboard — 122 pfmeaRiskRows interactive; stat cards (Total/Critical Sev9/High Sev8/Review/Open/Resolved); Agent strip filter (Sale/Designer/Factory/Install); Sev chips; Stage/Status/Review dropdowns; text search; sortable table; expand-row detail; per-row status selector (localStorage); CSV export | _knowledge-export.json (pfmeaRiskRows 122 rows), thai_ai_agent_evaluation_metrics.html | 2026-09-04 |
| n:166 | thai_monolith_platform_spec.html | HTML | MONOLITH Platform Architecture Spec — แยก Platform-level (MONOLITH Core) vs Client-level (Daph); 4-layer stack; 6 Core Components with P/C/S boundary; Platform vs Client Boundary table (20 rows); Daph Instance Profile; 10-step Onboarding; client_daph.json Config Schema | all prior Thai HTML specs (thai_mcp_layer, thai_automation_layer, thai_line_oa, thai_installation_agent) | 2026-09-04 |
| n:167 | thai_monolith_client_registry.html | HTML | Thai Brief | MONOLITH Client Registry — Dashboard แสดง Registered Clients: Daph (Active) + Swift (Pilot); stat cards (2 clients/11 agents/12 MCP); card+table view; component matrix; filter/search | Sarabun; navy/gold/green | All |
| n:166-mod | thai_monolith_platform_spec.html | HTML | Thai Brief | [UPDATED] เพิ่ม Tab 04B Swift Logistics instance: 5 agents/7-stage journey/PFMEA pilot/PWA Driver App/.env config/Daph vs Swift comparison table; 909L→1042L | Sarabun; navy/gold/green | All |
| n:168 | client_daph.json | JSON | Config | MONOLITH client config — client_id=daph; line{liff_ids×5, rich_menu_ids×5}; supabase{}; agents[6]; pfmea{122 rows, TC-INSTALL-02/05 score=4}; field_app{8 checklist}; branding{} | — | All |
| n:168 | validate_client_config.py | Python | Tool | Config validator — 129 checks (Top Level/LINE/Supabase/Agents/PFMEA/Field App/Branding); PASS/FAIL/WARN; exit 0=valid | — | All |

| n:168 | client_swift.json | JSON | Config | Swift Logistics client config — client_id=swift; domain=logistics_delivery; liff_ids×7; rich_menu_ids×6; agents[5]; pfmea{35 rows, TC-DRIVER-01/03 score=4}; field_app{CHK-D01–D10, accent #e65c00}; 127/127 PASS validated | — | 2026-09-04 |
| n:169 | thai_monolith_api_reference.html | HTML | Thai Brief | MONOLITH API Reference — 11 agents × 55 tools; Daph+Swift; searchable + client filter tabs (All/Daph/Swift); tool cards: endpoint/method/params/response/MCP event | — | 2026-09-04 |
| n:170 | thai_swift_driver_fieldapp.html | HTML | Thai Brief | Swift Driver PWA Field App — 6 screens (Job Queue/GPS Pickup/In Transit/POD Capture/Damage Report/Job Summary); TC-DRIVER-01 + TC-DRIVER-03 Sev:9 tagged; CHK-D01–D10; halt_alert trigger; accent #e65c00; SW swift-driver-v1 | — | 2026-09-04 |
| n:165-mod2 | thai_pfmea_live_dashboard.html | HTML (modified) | Thai Brief | [UPDATED] Client filter chips (Daph/Swift) + Swift agent cards (5) + 35 Swift PFMEA rows (ids 123–157); total 157 rows; fClient state + filteredRows() client logic | — | 2026-09-04 |
| n:166-mod2 | thai_monolith_platform_spec.html | HTML (modified) | Thai Brief | [UPDATED] Tab 07 MCP Event Catalog added: 12-event schema table + Daph/Swift routing rules tables + JSON examples; 8 tabs total | — | 2026-09-04 |
