...
| `0185` | Open audit findings — site_code-scoped tables (tracking record) | 74 operational tables missing org_id-scoped RLS policy; 4 CRITICAL (no RLS at all): `approval_decision`, `approval_request`, `capture_item`, `work_item`; remediation: Phase 2 RLS epic (migrations 0186–0194) |
| `0186` | Phase 2 RLS — 4 CRITICAL tables (batch 0) | `org_id` column + NOT NULL enforcement + `<table>_tenant_isolation` policies on `approval_decision`, `approval_request`, `capture_item`, `work_item`; clears all 4 tables with no RLS at all |
| `0187` | Phase 2 RLS — installation + production domain — 12 tables (batch 1) | `org_id`-scoped `<table>_tenant_isolation` policies on `installation_projects`, `installation_rooms`, `installation_tasks`, `installation_photos`, `installation_photo_annotations`, `installation_memberships`, `installation_field_reports`, `installation_approvals`, `installation_audit_log`, `installation_issues`, `installation_plans`, `production_milestones` |
| `0188` | Phase 2 RLS — factory domain — 5 tables (batch 2) | `org_id`-scoped `<table>_tenant_isolation` policies on `factory_checkins`, `factory_gate_config`, `factory_job_events`, `factory_jobs`, `factory_station_checklists`; config tables receive sentinel UUID; all direct reads via SECDEF RPCs |
| `0189` | Phase 2 RLS — LINE OA domain — 11 tables (batch 3) | `org_id`-scoped `<table>_tenant_isolation` policies on `line_bind_codes`, `line_groups`, `line_group_members`, `line_oa_audit_log`, `line_oa_channels`, `line_oa_conversations`, `line_oa_customer_identity`, `line_oa_inbound_messages`, `line_oa_message_templates`, `line_oa_orders`, `line_oa_outbound_messages`; `line_groups`+`line_bind_codes` backfilled from `installation_projects.org_id` (set by 0187) |
| `0190` | Phase 2 RLS — finance/accounting domain — 10 tables (batch 4) | `org_id`-scoped `<table>_tenant_isolation` policies on `bank_feed_txn`, `expense_category_map`, `finance_config`, `job_cost_config`, `job_cost_entries`, `journal_entry`, `journal_line`, `payment_installments`, `receipts`, `receivable` |
| `0191` | Phase 2 RLS — package/sales domain — 7 tables (batch 5) | `org_id`-scoped `<table>_tenant_isolation` policies on `package_addons`, `package_estimates`, `package_materials`, `package_stages`, `project_turnkey`; sentinel UUID policies on `price_rates`, `turnkey_offers` |
| `0192` | Phase 2 RLS — capture/documents domain — 7 tables (batch 6) | `org_id`-scoped `<table>_tenant_isolation` policies on `capture_artifact`, `capture_audit_log`, `contract_documents`, `document_links`; sentinel UUID policies on `capture_type_config`, `customer_docs`, `form_templates` |
| `0193` | Phase 2 RLS — site/field ops domain — 7 tables (batch 7) | `org_id`-scoped `<table>_tenant_isolation` policies on `appointments`, `daily_reports`, `qc_inspections`, `site_checkins`, `site_survey_zone`, `variation_orders`, `work_packages` |
| `0194` | Phase 2 RLS — operational misc domain — 11 tables (batch 8) | `org_id`-scoped + governance-scoped `<table>_tenant_isolation` policies on `design_lock_field_config`, `issue_routing`, `lead_followup_config`, `material_master`, `material_purchase_price`, `ops_contacts`, `phase_rosters`, `released_spec`, `revision_event`, `staff_bind_tokens`, `workflow_audit_log` |
...
| `supabase/tests/0187_installation_domain_rls.sql` | 0187 | T-0187-01 → T-0187-53 (53 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 12 installation/production tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `installation_projects`, `installation_rooms`, `production_milestones`; own-org access on `installation_projects` |
| `supabase/tests/0188_factory_domain_rls.sql` | 0188 | T-0188-01 → T-0188-25 (25 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 5 factory tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `factory_jobs`, `factory_checkins`, `factory_job_events`; own-org access on `factory_jobs` |
| `supabase/tests/0189_line_oa_domain_rls.sql` | 0189 | T-0189-01 → T-0189-49 (49 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 11 LINE OA tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `line_oa_conversations`, `line_groups`, `line_bind_codes`; own-org access on `line_groups` |
| `supabase/tests/0190_finance_accounting_domain_rls.sql` | 0190 | T-0190-01 → T-0190-45 (45 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 10 finance/accounting tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation; own-org access |
| `supabase/tests/0191_package_sales_domain_rls.sql` | 0191 | T-0191-01 → T-0191-35 (35 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 7 package/sales tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `package_addons`, `package_stages`, `project_turnkey`; own-org access on `project_turnkey` |
| `supabase/tests/0192_capture_documents_domain_rls.sql` | 0192 | T-0192-01 → T-0192-35 (35 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 7 capture/documents tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `capture_artifact`, `contract_documents`; own-org access on `capture_artifact` |
| `supabase/tests/0193_site_field_ops_domain_rls.sql` | 0193 | T-0193-01 → T-0193-35 (35 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 7 site/field ops tables; `<table>_tenant_isolation` policy existence; cross-tenant SELECT isolation on `appointments`, `site_checkins`, `work_packages`; own-org access on `work_packages` |
| `supabase/tests/0194_operational_misc_domain_rls.sql` | 0194 | T-0194-01 → T-0194-51 (51 tests) | `has_column` + `col_not_null` + `relrowsecurity` on 11 operational misc tables; `<table>_tenant_isolation` policy existence; governance-scoped isolation on `revision_event`, `material_purchase_price`, `workflow_audit_log`, `staff_bind_tokens`; cross-tenant isolation on `phase_rosters`; sentinel visibility for `issue_routing`, `released_spec` |

**Total pgTAP tests authored (forward migrations):** 14 + 35 + 17 + 18 + 13 + 13 + 16 + 20 + 53 + 25 + 49 + 45 + 35 + 35 + 35 + 51 = **474 tests** (0179 F1: 14, 0179 NNB: 35, 0180: 17, 0181: 18, 0182: 13, 0183: 13, 0176 notification_preferences: 16, 0186 critical_tables: 20, 0187 installation_domain: 53, 0188 factory_domain: 25, 0189 line_oa_domain: 49, 0190 finance_accounting_domain: 45, 0191 package_sales_domain: 35, 0192 capture_documents_domain: 35, 0193 site_field_ops_domain: 35, 0194 operational_misc_domain: 51)

**Grand total pgTAP assertions (pg_prove SQL suites):** 474 (forward) + 12 (rollback) + 25 (cross-tenant) = **511 tests**
...
*Last updated: 2026-09-01 — pgTAP SQL total 511 (forward 474 + rollback 12 + cross-tenant 25); Phase 2 RLS epic complete — 0190–0194 added; 0194_operational_misc_domain_rls.sql added — Phase 2 batch 8, 51 pgTAP tests (T-0194-01–T-0194-51) covering 11 operational misc tables; 0193_site_field_ops_domain_rls.sql added — Phase 2 batch 7, 35 pgTAP tests covering 7 site/field ops tables; 0192_capture_documents_domain_rls.sql added — Phase 2 batch 6, 35 pgTAP tests covering 7 capture/documents tables; 0191_package_sales_domain_rls.sql added — Phase 2 batch 5, 35 pgTAP tests covering 7 package/sales tables; 0190_finance_accounting_domain_rls.sql added — Phase 2 batch 4, 45 pgTAP tests covering 10 finance/accounting tables; 0 tables remaining in Phase 2 RLS epic; previous totals: pgTAP SQL total 310 (forward 273 + rollback 12 + cross-tenant 25) after 0189.*
