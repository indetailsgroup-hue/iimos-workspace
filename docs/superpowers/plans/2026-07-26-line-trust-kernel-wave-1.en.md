# LINE Trust Kernel Wave 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the canonical Trust Kernel contracts, additive tenant bridge, policy-decision record, and verified LINE shadow observer without changing Daph business outcomes or sending live messages.

**Architecture:** PostgreSQL remains the authoritative enforcement and audit boundary. Pure TypeScript contracts make the decision vocabulary deterministic and testable, while additive migrations introduce canonical Tenant–Organization–Site relationships beside the current `site_code` model. The existing `rpc_ingest_line_webhook` remains authoritative in Wave 1; a second signature-verifying observer records shadow decisions only and cannot write domain state or delivery intent.

**Tech Stack:** TypeScript 5.2, Vitest 3, fast-check 4, Supabase Edge Functions on Deno, PostgreSQL/PLpgSQL, Supabase CLI, Python 3 with pytest 8/Hypothesis 6/psycopg 3, Node.js ESM.

## Global Constraints

- Execute in an isolated nested-repository worktree created with `using-git-worktrees`; do not work in the current dirty checkout.
- Re-read the parent `CONTEXT.md` and 21 July 2026 repository-scope correction before current-state claims.
- Preserve the parent and nested dirty worktrees.
- Plan baseline is the approved design commit `5ff835145d91fee403c3a19f1f01ef20459a052f`; rebase the execution worktree before Task 1 if the chosen base changes.
- Migrations `0162`, `0163`, and `0164` are reserved for this wave. If the execution base already contains one of these numbers, stop before editing and reissue all three numbers as one reviewed plan correction.
- LINE remains the Human Surface, not the system of record or authorization authority.
- Daph is one pilot tenant, not the platform owner.
- `site_code` remains readable and writable for compatibility in Wave 1, but every shadow decision records the canonical tenant resolution.
- Wave 1 is shadow-only: do not change a legacy business outcome, create an outbound delivery intent, or enable Tenant-2 live messaging.
- Default policy outcome is `DENY`; unknown actors may receive `QUARANTINE` only for `evidence.submit`.
- Delegation is explicit, expiring, revocable, non-transitive, and cannot amplify authority.
- Never log channel secrets, access tokens, bind tokens, authorization codes, raw ID tokens, or unnecessary PII.
- Use TDD for every code or SQL behavior.
- Every task ends with targeted verification and a focused commit.
- Project-facing Markdown must have aligned English and Thai editions with matching standalone HTML.
...
- `supabase/functions/_shared/trust-kernel/types.ts` — canonical decision and envelope types.
- `supabase/functions/_shared/trust-kernel/policy.ts` — pure fail-closed policy evaluator.
- `supabase/functions/_shared/trust-kernel/policy.test.ts` — unit and table-driven policy tests.
- `supabase/migrations/0162_line_trust_tenant_foundation.sql` — canonical tenant structures and additive Daph mapping.
- `supabase/migrations/0163_line_trust_policy_decision.sql` — action catalog, policy decision ledger, authorization RPC.
- `supabase/migrations/0164_line_trust_shadow_ingress.sql` — signature-verifying shadow inbox and observer RPC.
- `tests/line-oa-commerce/py/test_tenant_boundary_property.py` — tenant and mapping denial properties.
- `tests/line-oa-commerce/py/test_trust_policy_property.py` — database policy properties.
- `tests/line-oa-commerce/py/test_shadow_ingress_property.py` — idempotent observer and no-delivery properties.
- `supabase/functions/line-webhook/index.test.ts` — transport-level shadow wiring tests.
- `scripts/line-trust-shadow-report.mjs` — machine-readable Wave 1 gate report.
- `scripts/__tests__/lineTrustShadowReport.test.ts` — report aggregation tests.
...
- `supabase/functions/line-webhook/index.ts:35-122` — inject and invoke the shadow observer only after verified legacy acceptance.
- `package.json` — add focused `test:line-trust` and `report:line-trust-shadow` commands.

### Explicitly do not modify in Wave 1
...
- existing migrations `00000000000000` through `0161`
- LINE channel webhook configuration
- any live customer delivery configuration
...
**Interfaces:**
- Consumes: tracked file paths from `git ls-files` and UTF-8 source text.
- Produces: `summarizeTrustBaseline(entries: SourceEntry[]): TrustBaseline`.
- Produces CLI artifact: `artifacts/line-trust/wave-1-baseline.json`.
...
Expected: exit `0`; `node_modules/.bin/vitest` and `node_modules/.bin/tsc` exist. This proves dependency installation only.
...
Expected: FAIL because `scripts/line-trust-baseline.mjs` does not exist.

- [ ] **Step 4: Implement the deterministic baseline generator**
...
- [ ] **Step 5: Run the test and generate the baseline**
...
Expected: test PASS; report command exits `0`; JSON lists both current ingress entrypoints and records current `tenant_id`/`site_code` evidence without claiming production readiness.
...
**Interfaces:**
- Consumes: `TrustEnvelope` and resolved `TrustContext`.
- Produces: `evaluateTrustPolicy(envelope, context): TrustDecisionResult`.
...
- [ ] **Step 4: Implement the fail-closed evaluator**
...
Expected: policy tests PASS; TypeScript exits `0`.
...
**Interfaces:**
- Produces canonical tables: `tenants`, `organizations`, `sites`, `auth_subjects`, `tenant_profiles`, `tenant_memberships`, `project_parties`, `access_grants`, `trust_delegations`, `line_identity_bindings`, `site_code_mappings`.
- Adds nullable bridge columns to current LINE/project rows; Wave 1 does not remove `site_code`.
- Produces `resolve_tenant_from_site_code(text)` and `current_tenant_id()`.

- [ ] **Step 1: Write the failing database property test**
...
- [ ] **Step 2: Reset the local database and verify the red state**
...
Expected: FAIL because the canonical tables and resolver do not exist.

- [ ] **Step 3: Create the additive foundation migration**

Create `supabase/migrations/0162_line_trust_tenant_foundation.sql` with these exact invariants:
...
Do not add `NOT NULL` to bridge columns in Wave 1. Unresolved rows must remain visible to the shadow report and block enforcement.

- [ ] **Step 4: Reset the database and run the tenant test**
...
Expected: PASS; existing active site codes resolve to the fixed Daph pilot tenant; cross-tenant project-party insertion fails.

- [ ] **Step 5: Run existing schema and RLS regression tests**

Run from `tests/line-oa-commerce/py`:

```bash
python -m pytest -q test_schema_structure_smoke.py test_rls_read_scoping_property.py
```

Expected: PASS or documented skips only for unavailable external prerequisites; no failed test.
...
### Task 4: Database Policy Decision and Immutable Decision Record
...
**Interfaces:**
- Consumes: `rpc_authorize_business_action(p_envelope jsonb)`.
- Produces JSON keys: `decision`, `reason_codes`, `required_assurance`, `decision_id`, `policy_version`.
- Writes exactly one `policy_decisions` row per call.

- [ ] **Step 1: Write the failing decision property tests**
...
Expected: FAIL because `rpc_authorize_business_action` and `policy_decisions` do not exist.

- [ ] **Step 3: Implement action catalog, decision ledger, and RPC**
...
- [ ] **Step 4: Reset and run policy tests**
...
Expected: PASS; unknown actors only quarantine evidence; tenant mismatch denies and records one immutable decision.
...
**Interfaces:**
- Consumes: `rpc_line_shadow_observe_webhook(channel_identifier, raw_body, signature)`.
- Produces: `{accepted, received, duplicate, unresolved, decisions}`.
- Writes only `line_inbound_events`, `policy_decisions`, and `line_shadow_observations`.
- Must never write `line_oa_outbound_messages`, `notification`, orders, approvals, workflow state, or project state.

- [ ] **Step 1: Write the failing no-side-effect property test**
...
Expected: FAIL because the shadow observer RPC does not exist, or SKIP only when the signed fixture variables are absent.

- [ ] **Step 3: Implement the shadow-only inbox and observer**
...
Add `event.observe` to `trust_action_catalog` in the same migration:
...
- [ ] **Step 4: Reset and run shadow tests plus existing ingestion properties**
...
Expected: PASS or explicit signed-fixture SKIP; no failure; outbound row count remains unchanged by shadow observation.
...
### Task 6: Wire Shadow Observation Behind Legacy Acceptance
...
**Interfaces:**
- Adds `ShadowObserveFn(args: IngestArgs): Promise<ShadowObserveResult>`.
- Adds `ShadowLogger.error(message: string): void`.
- `handleLineWebhook(req, ingest, observe, logger)` preserves existing HTTP status/body.
...
Expected: FAIL because `handleLineWebhook` does not accept observer/logger dependencies.

- [ ] **Step 3: Add the injected observer contract**

Add after `IngestFn` in `supabase/functions/line-webhook/index.ts`:
...
Replace the accepted branch with:
...
Add the default observer beside `defaultIngest`:
...
Do not expose shadow details in the LINE response and do not include raw database errors in logs.

- [ ] **Step 4: Run transport and existing LINE tests**
...
Expected: PASS; existing approval handler remains unchanged; legacy response mapping is preserved.
...
### Task 7: Shadow Gate Report and Bilingual Runbook

**Files:**
- Create: `scripts/line-trust-shadow-report.mjs`
- Create: `scripts/__tests__/lineTrustShadowReport.test.ts`
- Modify: `package.json`
- Create: four Wave 1 runbook editions listed in the file map.

**Interfaces:**
- Produces `buildShadowReport(rows): ShadowReport`.
- CLI writes `artifacts/line-trust/wave-1-shadow-report.json`.
- Gate fields: unresolved mappings, observed events, tenant mismatches, outcome counts, outbound side effects, pass/fail.
...
- [ ] **Step 3: Implement the report builder and CLI**
...
- [ ] **Step 4: Write the aligned runbook editions**

Create both Markdown editions with these exact sections:
...
Translate the complete content into Thai in `wave-1-shadow.th.md`. Generate standalone HTML from each Markdown file and verify headings and body text match their source.

- [ ] **Step 5: Run report tests and render verification**
...
Expected: report test PASS; the report command exits `0` only when observed events are non-zero and unresolved, mismatch, and outbound side effects are all zero.
...
### Task 8: Wave 1 Complete Verification and Review Gate

**Files:**
- Verify all files from Tasks 1–7.
- Do not create or modify production code in this task.

**Interfaces:**
- Consumes the complete Wave 1 diff and machine-readable reports.
- Produces a reviewable verification record; does not enable enforcement.

- [ ] **Step 1: Verify both Git roots and exact Wave 1 scope**

Run in the parent root and then the isolated nested worktree:
...
Expected: the parent root is unchanged by Wave 1; the isolated nested worktree contains only intentional Wave 1 changes.

- [ ] **Step 2: Run the complete targeted TypeScript suite**
...
Expected: exit `0` with the final Vitest summary showing zero failed tests. Partial or truncated output is UNKNOWN.

- [ ] **Step 3: Run the database reset and complete Wave 1 database suite**
...
Expected: exit `0`, zero failures, and only prerequisite-specific skips explicitly listed in the final pytest summary.
...
Expected: both commands exit `0`; complete output includes the final compiler/build summary.

- [ ] **Step 5: Generate and inspect evidence**
...
- baseline JSON names both pre-existing ingress paths;
- shadow report `pass` is `true`;
- unresolved mappings, tenant mismatches, and outbound side effects are `0`;
- `git diff --check` exits `0`;
- no unrelated file is staged.

- [ ] **Step 6: Request code review**

Use `requesting-code-review` with the approved design, this plan, the complete diff, test output, migration reset output, build output, and both JSON reports.

Expected: no unresolved Critical or High review finding.

- [ ] **Step 7: Commit review corrections and stop at the Wave 1 gate**

Commit each accepted correction by concern. Do not squash away evidence-bearing commits before review.

Wave 1 is complete only when:

1. all Task 8 commands have fresh complete successful output;
2. the shadow report passes;
3. review has no open Critical/High finding;
4. live behavior and live Tenant-2 delivery remain unchanged;
5. the owner approves transition to the separately reviewed Wave 2 plan.

---

## Execution handoff

Plan complete and saved to `docs/superpowers/plans/2026-07-26-line-trust-kernel-wave-1.en.md`.

Two execution options:

1. **Subagent-Driven (recommended):** use `subagent-driven-development`, one fresh implementation worker per task with specification and quality review between tasks.
2. **Inline Execution:** use `executing-plans`, execute in batches with explicit checkpoints.

No execution begins until the user chooses an option.
