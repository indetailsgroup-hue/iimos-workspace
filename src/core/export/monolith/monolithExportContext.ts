/**
 * iimoExportContext.ts - MONOLITH Export Context Types
 *
 * ARCHITECTURE:
 * - Define the context interface that MONOLITH provides to exporters
 * - Context contains all resolved data needed for export
 * - Must be deterministic: same job state → same context
 *
 * INTEGRATION:
 * - Context provider extracts data from MONOLITH runtime (cabinets, nesting, materials)
...
/**
 * MONOLITH export context provider
 *
 * Implementations extract context from MONOLITH runtime.
 * The provider MUST return deterministic context.
 */
export interface MONOLITHExportContextProvider {
  /**
 ...
 */
export function createStubContextProvider(): MONOLITHExportContextProvider {
  return {
